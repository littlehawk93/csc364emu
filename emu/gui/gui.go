...
func NewGui(emu *emu.Emulator, speed int) (*EmulatorGUI, error) {
...
func (me *EmulatorGUI) renderEmulator(g *gocui.Gui) error {
	...
	fmt.Fprintf(clockView, "Clock: %12d", me.emulator.GetClock())
	...
	for i := 0; i < emu.HWRegistersCount/2; i++ {

		fmt.Fprintf(registersView, " %s  | %04X | %s | %04X \n", getRegisterTitle(i), me.emulator.GetRegister(byte(i)), getRegisterTitle(i+8), me.emulator.GetRegister(byte(i+8)))
	}
	...
	if me.emulator.GetRegister(15) < 0xFFFF {
		fmt.Fprintf(instructionView, "   %1X   %1X   %1X   %1X   ", byte(me.emulator.ROM[me.emulator.GetRegister(15)]>>12&0x000F), byte(me.emulator.ROM[me.emulator.GetRegister(15)]>>8&0x000F), byte(me.emulator.ROM[me.emulator.GetRegister(15)]>>4&0x000F), byte(me.emulator.ROM[me.emulator.GetRegister(15)]&0x000F))
	} else {
	...
	romAddress := int(me.emulator.GetRegister(15))
...
func getRegisterTitle(index int) string {
	...
	if index == emu.RegisterInput {
		return "INPUT"
	} else if index == emu.RegisterOutput1 {
		return "OUT 1"
	} else if index == emu.RegisterOutput2 {
		return "OUT 2"
	} else if index == emu.RegisterProgramCounter {
		return "PROGC"
	} else {
...
func (me *EmulatorGUI) startEmulator(g *gocui.Gui, v *gocui.View) error {
	...
	if !me.emulatorStarted {
		...
		go me.emulator.Begin(me.speed, me.updateLayout)
...
func (me *EmulatorGUI) updateLayout(emu *emu.Emulator, err error) {
